...
uint32_t inst_cache_hit(uint32_t address)//判断是否命中
{
    ...
    //此时内存的设置为  24位tag 3位组号 5位块内字节偏移量
    uint32_t sets_index = (address<<(27-isets))>>(32-isets);//得到3位的组号
    ...
    for (i = 0; i < iline; i++)//检查对应组中的四行
    {
        ...
        if (!(tag ^ (address>> (isets+5))))//tag 位相同再检查 valid
        {
...
uint32_t inst_cache_miss(uint32_t address)
{
    ...
    //将取出的块放入cache中对应的组
    uint32_t sets_index = (address<<(27-isets))>>(32-isets);    //计算组号
    ...
    for (j = 0; j < iline; j++)//有空行则将块放入
    {
        if (iCache.sets[sets_index].lines[j].valid == 0)//空位
        {
            ...
            iCache.sets[sets_index].lines[j].tag = address>> (isets+5);    // 更新tag
...
uint32_t data_cache_read(uint32_t address)
{
    ...
    if (dCache.sets[sets_index].lines[max_LRU_line].dirty == 1)//需要写回
    {
        ...
        uint32_t block_address = (dCache.sets[sets_index].lines[max_LRU_line].tag << (dsets+5)) | sets_index_13;
        for (i = 0; i < 8; i++)
