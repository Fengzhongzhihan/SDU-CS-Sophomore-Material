...
void teacell_score::on_myCourse_clicked()
{
...
void teacell_score::on_showScore_clicked()
{
...
void teacell_score::on_updateScore_clicked()
{
    int r = ui->tableView->currentIndex().row();
    if(r<0)
    {
    ...
    else
    {
        //
        QString id=m_model.item(r,0)->text();
        QString cname=m_model.item(r,1)->text();
        QString sid= m_model.item(r,2)->text();
        QString name =m_model .item(r,3)->text();
        QString firstName= m_model.headerData(1,Qt::Horizontal, Qt::DisplayRole).toString();

        if(firstName=="课程名")
        {
            teacell_updateStuInfo *updateStu= new teacell_updateStuInfo();
            updateStu->setSid(sid);
            updateStu->setName(name);
            updateStu->setCourse(cname);
...
void teacell_score::on_countScore_clicked()
{
    //获取第一列表头名
   QString firstName= m_model.headerData(1,Qt::Horizontal, Qt::DisplayRole).toString();
   if(firstName!="课程名")
   ...
   //得到课程号
   QString id= m_model.item(0,0)->text();
...
void teacell_score::on_printScore_clicked()
{
    //获取第一列表头名
   QString firstName= m_model.headerData(1,Qt::Horizontal, Qt::DisplayRole).toString();
   if(firstName!="课程名")
    ...
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save as..."),
         ...
         if (fileName!="")
         {
             ...
             if (excel->setControl("Excel.Application")) //连接Excel控件
             {
                 ...
                 cell->dynamicCall("SetValue(const QString&)", "班级成绩信息");
