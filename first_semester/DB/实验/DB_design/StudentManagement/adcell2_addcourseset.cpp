...
#include "ui_adcell2_addcourseset.h"
#include<sqlop.h>

adcell2_addCourseSet::adcell2_addCourseSet(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::adcell2_addCourseSet)
{
    ui->setupUi(this);
}

adcell2_addCourseSet::~adcell2_addCourseSet()
{
    delete ui;
}

void adcell2_addCourseSet::on_cancelButton_clicked()
{
    this->close();
}

void adcell2_addCourseSet::on_commitButton_clicked()
{
    QString courseSet_id = ui->courseSet_id->text();
    ...
    if(courseSet_id == "")
    {
        QMessageBox::warning(this,"错误","请输入课程集编号");
    }
    else if(teacher_id == "")
    {
    ...
    else if(sqlOP::getInstance()->isCourseSetExist(courseSet_id))
    {
        QMessageBox::warning(this,"错误","已存在使用该id的课程集");
    }
    ...
    else
    {
        ...
        sqlOP::getInstance()->addCourseSetInfos(info);
        this->close();
    }
}
