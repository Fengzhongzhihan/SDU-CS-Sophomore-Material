...
adcell1_student::adcell1_student(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::adcell1_student)
{
    ui->setupUi(this);
...
adcell1_student::~adcell1_student()
{
    delete ui;
}

void adcell1_student::on_btn_search_clicked()
{
    ...
    {
        strCondition = QString( "where student_id REGEXP '%1' or student_name REGEXP '%1'or age = '%1' or grade = '%1' or classes = '%1' or identity_id REGEXP '%1' or sex = '%1' or major_name REGEXP '%1' or college_name REGEXP '%1'").arg(stuInfo);
    }
    initPage(strCondition);
}

void adcell1_student::initPage(QString strCondition)
{
...
void adcell1_student::on_pushButton_clicked()
{
    //获取第一列表头名
    ...
    QString fileName = QFileDialog::getSaveFileName(this, tr("Save as..."),
         ...
         if (fileName!="")
         {
             ...
             if (excel->setControl("Excel.Application")) //连接Excel控件
             {
                 ...
                 cell->dynamicCall("SetValue(const QString&)", "学生名单");
...
void adcell1_student::on_btn_deletestu_clicked()
{
    int r = ui->tableView->currentIndex().row();
    if(r<0)
    {
        QMessageBox::warning(this,"错误","请选择你想要删除的学生!");
    }
    else
    {
        QString student_id=m_model.item(r,0)->text();
        sqlOP::getInstance()->deleteStudentInfos(student_id);
    }
    on_btn_search_clicked();
}

void adcell1_student::on_btn_addstu_clicked()
{
    adcell2_addStudent * addStu = new adcell2_addStudent(this);
    addStu->exec();
    on_btn_search_clicked();
}

void adcell1_student::on_btn_updatastu_clicked()
{
    int r = ui->tableView->currentIndex().row();
    if(r<0)
    {
        QMessageBox::warning(this,"错误","请选择你想要修改的学生记录!");
    }
    else
    {
        QString student_id = m_model.item(r,0)->text();
        adcell2_updateStudent* upStudent =new adcell2_updateStudent(this);
        upStudent->setStudentId(student_id);
        upStudent->exec();
        on_btn_search_clicked();
    }
}
