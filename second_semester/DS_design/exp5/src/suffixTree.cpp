...
void SuffixTree::create(string str){//对字符串str构造后缀树
    ...
    while (index < str.length()) {// 循环建立后缀
        ...
        char w =str[currentIndex];// 得到当前的后缀字符 abcabcx   依此为a b c a b c x
        bool f=find(w); // 查找这棵树中有无
        //存在,则remainder++即可
        ...
        //此时index=null, 且需要插入额外后缀（remainder=0）
        ...
        // 找不到而且reminder==0表示之前在该字符之前未剩余有其他待插入的后缀字符，所以直接插入该后缀字符即可
            ...
            node->init(tmp);
            node->iiflag=iflag++;
            SuffixNode* child = activenode->point->Child; //插入point的最后一个孩子
            if (NULL== child) {
            ...
            splitNode->str = splitNode->str.substr(0,activenode->length);// 修改当前节点的字符为0~length的字符
...
void SuffixTree::Split(string str,int currentIndex,SuffixNode *prefixNode){//相当于把分割部分单独写出来
    ...
    // 此处计算剩余待插入的后缀的开始位置，例如剩余后缀数为2，已处理过abx，需要处理bx和x时，下面计算b，相当于新分支节点
    ...
    // 从dealStart开始查找所有后缀字符是否都存在(相对与活动点) // bx x
...
bool SuffixTree::find(char w){
    ...
    if(current==NULL){//current为空，说明无活动边，则在其子节点中进行查找, 如 root, null, 1
        //寻找遍历子节点, 只找一层
