...
void exec()
{
   ...
   // 创建了一个指向 Object 对象的指针 a，并通过 new 关键字动态分配了内存来创建一个新的 Object 对象。此时 count 值增加，因为构造函数被调用。
//接着，又创建了一个匿名的 Object 对象，并通过引用 b 引用了这个对象, 将两者关联起来, 这个对象通过复制构造函数从指针 a 拷贝而来，然后 count 值再次增加，因为构造函数被调用。
