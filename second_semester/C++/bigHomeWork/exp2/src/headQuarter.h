...
void headQuarter::makeWarrior() {
    ...
    if (makingSeq[curMakingIndex] == 1) {//如果是dragon
        dragon curWarrior(curId, lifeValue, totalLifeValue);
        curWarrior.print();
    } else if (makingSeq[curMakingIndex] == 2) {//如果是ninja
        ninja curWarrior(curId, lifeValue);
        curWarrior.print();
    } else if (makingSeq[curMakingIndex] == 3) {//如果是iceman
        iceman curWarrior(curId, lifeValue);
        curWarrior.print();
    } else if (makingSeq[curMakingIndex] == 4) {//如果是lion
        lion curWarrior(curId, lifeValue, totalLifeValue);
        curWarrior.print();
